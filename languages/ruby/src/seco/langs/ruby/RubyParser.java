...
public class RubyParser extends NBParser
{
    ...
    {
        super(support);
        engine = (JRubyScriptEngine) support.getDocument()
                .getEvaluationContext().getEngine("jruby");
    }
    ...
    public Object resolveVar(String s, int offset) // throws EvalError
    {
        ...
        if (getRootNode() != null)
        {
            ...
            try
            {
                TypeAnalyzer a = new TypeAnalyzer(getRootNode(), n, offset - 1,
                        support.getDocument(), this);
                String type = a.getType(s);
                System.out.println("Type: " + s + ":" + type);
                if (type == null) type = a.expressionType(n);
                System.out.println("Type1: " + n + ":" + type);
                if (type != null)
                {
                    if (type.indexOf('.') > 0)
                    {
                        Class[] info = ClassRepository.getInstance().findClass(
                                type);
    ...
    private Object get_seco_global_var(String s)
    {
        if(!s.startsWith("$")) return null;
        s = s.substring(1);
        boolean simple = (s.indexOf('.') < 0);
        String lead =  simple ? s : s.substring(0, s.indexOf('.'));
        Object glob = engine.getContext().getAttribute(lead);
        if(simple && glob != null) return glob;
        //TODO: add composite analyzer
        return null;
    }
    ...
    Object evalType(String name)
    {
        ...
        String s = "Kernel.const_get('" + name + "')";
        try
        {
            return engine.eval(s, engine.getContext());
        }
        ...
        parserRunnable = new NBParser.ParserRunnable() {
            public boolean doJob()
            {
                ...
                try
                {
                    ...
                    nodes.clear();
                    astRoot = (RootNode) runtime.getParser()
                            .parse(
                                    "<unknown>",
                                    new ByteArrayInputStream(seg.toString()
                                            .getBytes()),
                                    null,
                                    new ParserConfiguration(runtime.getKCode(),
                                            0, true, false, true, runtime
                                                    .getInstanceConfig()));
                    // seg.toString(), "<unknown>", null, 0, true);
    ...
    // //if (completeStrings(proposals, index, prefix, astOffset, lexOffset,
    // th))
    // //{
    // // return proposals;
    // //}
