...
public class GraphicsUtil {
    ...
    static {
        boolean warn = true;
        try {
            String s = System.getProperty("ch.randelshofer.quaqua.ext.batik.warn_destination", "true");
...
// END PATCH W. Randelshofer Performance
    ...
    public static ColorModel coerceData(WritableRaster wr, ColorModel cm, boolean newAlphaPreMult) {
        ...
        //                    " Out: " + newAlphaPreMult);
        if (!cm.hasAlpha()) // Nothing to do no alpha channel
        {
            return cm;
        }

        if (cm.isAlphaPremultiplied() == newAlphaPreMult) // nothing to do alpha state matches...
        {
            return cm;
        }
