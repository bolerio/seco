...
public class JTreeASTModel implements TreeModel
{
    ...
    public Object getChild(Object parent, int index)
    {
        ...
        if (index >= 0 && index < p.children.length) return p.getChild(index);
    ...
    {
        if (parent == null) { throw new IllegalArgumentException("root is null"); }
    ...
    public SimpleNode[] getTreePathNodes(SimpleNode root, SimpleNode node,
            int depth)
    {
        ...
        else
        {
            ...
            {
                retNodes = getTreePathNodes((SimpleNode) getChild(root, i),
                        node, depth);
                if (retNodes != null)
                {
                    retNodes[depth - 1] = root;
