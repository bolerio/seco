...
public class CellUtils
{
	...
	private static void processGroup(CellGroup group)
	{
		for (int i = 0; i < group.getArity(); i++)
		{
			...
			if (cgm == null || cgm instanceof Cell)
				continue;
			CellVisual visual = CellUtils.getVisual(cgm);
			if (visual instanceof NBUIVisual)
				evalInitCells((CellGroup) cgm);
	...
	public static void evalInitCells(CellGroup group)
	{
		...
		if (name == null)
			name = ThisNiche.defaultLanguage();
		evalInitGroup(group, name, ThisNiche.getTopContext(),
				CellUtils.isInitCell(group));
	}

	public static void evalInitGroup(CellGroup group,
			String inherited_engine_name, EvaluationContext ctx,
			boolean inherited_init)
	{
		String name = CellUtils.getEngine(group);
		if (name == null)
			name = inherited_engine_name;
		if (name == null)
			name = ThisNiche.defaultLanguage();
		...
		for (int i = 0; i < group.getArity(); i++)
		{
			...
			if (!init && inherited_init)
				init = true;
			if (cgm instanceof CellGroup)
				evalInitGroup((CellGroup) cgm, name, ctx, init);
	...
	public static void evalGroup(CellGroup group, String inherited_engine_name,
			EvaluationContext ctx)
	{
		...
		for (int i = 0; i < group.getArity(); i++)
		{
			CellGroupMember cgm = group.getElement(i);
			if (cgm instanceof CellGroup)
				evalGroup((CellGroup) cgm, name, ctx);
			else if (CellUtils.isInputCell(cgm))
				evalCell(group, (Cell) cgm, name, ctx);
		}
	}

	public static void evalCell(Cell cell, String inherited_engine_name,
			EvaluationContext ctx)
	{
			...
			String inherited_engine_name, EvaluationContext ctx)
	{
		...
		if (list.isEmpty())
		{
			...
			CellGroupChangeEvent ev = new CellGroupChangeEvent(
					ThisNiche.handleOf(parent), parent.indexOf(cellH) + 1,
	...
	public static CellVisual getVisual(CellGroupMember c)
	{
		...
		if (visH == null
				|| visH.equals(ThisNiche.graph.getHandleFactory().nullHandle()))
		{
	...
	{
		return hg.findOne(ThisNiche.graph,
				hg.and(hg.incident(h), hg.type(DescriptionLink.class)));
	}
	...
	{
		return m instanceof Cell && ((Cell) m).getValue() instanceof Scriptlet;
	}
	...
	public static void setBoolAttribute(CellGroupMember c, Object attr_name,
			boolean value)
	{
		Object attr = c.getAttribute(attr_name);
		boolean b = attr != null ? ((Boolean) attr).booleanValue() : false;
		if (value != b)
			c.setAttribute(attr_name, value);
	}
	...
	public static String getNotNullEngine(CellGroupMember c)
	{
		...
		return eng != null ? eng : ThisNiche.defaultLanguage();
	}
	...
	public static List<HGHandle> getOutCellHandles(HGHandle cellH)
	{
		...
		try
		{
			List<EventPubSub> subscriptions = hg.getAll(ThisNiche.graph, hg.and(
					hg.type(EventPubSub.class),
					hg.incident(EvalCellEvent.HANDLE), hg.incident(cellH),
					hg.orderedLink(new HGHandle[] { EvalCellEvent.HANDLE, cellH,
	...
	public static List<Cell> getOutCells(HGHandle cellH)
	{
		...
		if (cellH == null)
			return null;
		List<EventPubSub> subscriptions = hg.getAll(ThisNiche.graph, hg.and(
				hg.type(EventPubSub.class), hg.incident(EvalCellEvent.HANDLE),
				hg.incident(cellH),
				hg.orderedLink(new HGHandle[] { EvalCellEvent.HANDLE, cellH,
						ThisNiche.graph.getHandleFactory()
								.anyHandle(),
	...
	{
		return createOutputCellH(par, res.getText(), res.getComponent(),
				res.isError());
	}
	...
	{
		HGHandle h = (comp == null) ? ThisNiche.handleOf(text)
	...
	{
		List<HGHandle> set = hg.findAll(ThisNiche.graph, hg.and(
				hg.type(EventPubSub.class), hg.incident(EvalCellEvent.HANDLE),
				hg.incident(cell_handle),
			...
			Map<HGHandle, HGHandle> cells)
	{
		...
		for (int i = 0; i < in.getArity(); i++)
		{
			...
			if (isInputCell(m))
				...
				copyH = connect_output_cells((CellGroup) m, cells);
			else
			{
	...
	{
		List<EventPubSub> subs = hg
				.getAll(ThisNiche.graph,
						hg.and(hg.type(EventPubSub.class), hg.incident(h),
								hg.orderedLink(
										EvalCellEvent.HANDLE, ThisNiche.graph
	...
	public static void removeEventPubSub(HGHandle eventType, HGHandle publisher,
			HGHandle subscriber, HGHandle listener)
	{
		List<HGHandle> subscriptions = getEventPubSubListH(eventType, publisher,
				subscriber, listener);
			...
			HGHandle publisher, HGHandle subscriber, HGHandle listener)
	{
		return hg.getAll(ThisNiche.graph,
				hg.and(hg.type(EventPubSub.class), hg.incident(eventType),
						hg.incident(publisher), hg.orderedLink(new HGHandle[] {
			...
			HGHandle publisher, HGHandle subscriber, HGHandle listener)
	{
		return hg
				.findAll(
						ThisNiche.graph, hg
								.and(hg.type(EventPubSub.class),
										hg.incident(eventType),
										hg.incident(publisher),
			...
			HGHandle copyH)
	{
		...
		for (int i = 0; i < master.getArity(); i++)
			if (master.getElement(i) instanceof CellGroup)
				addCellGroupCopyListeners(master.getTargetAt(i),
						copy.getTargetAt(i));
			else
				addCellCopyListeners(master.getTargetAt(i),
						copy.getTargetAt(i));
	}
	...
	private static void remove_event_handlers(HGHandle masterH)
	{
		...
		List<HGHandle> subs = hg.findAll(ThisNiche.graph, hg.and(
				hg.type(EventPubSub.class), hg.incident(masterH),
				hg.orderedLink(new HGHandle[] { any, masterH, any, any })));
		for (HGHandle s : subs)
			ThisNiche.graph.remove(s, true);
		subs = hg.findAll(ThisNiche.graph, hg.and(hg.type(EventPubSub.class),
				hg.incident(masterH),
				hg.orderedLink(new HGHandle[] { any, any, masterH, any })));
		for (HGHandle s : subs)
			ThisNiche.graph.remove(s, true);
	}
	...
	{
		HGHandle h = hg.findOne(ThisNiche.graph,
				hg.and(hg.type(BackupLink.class), hg.incident(cell)));
	...
	private static void restoreFromLink(BackupLink link, boolean fully)
	{
		...
		{
			if (!fully && filter_all_but_eval_eps(inf))
				continue;
			addEventPubSub(inf.getEventType(), link.getCell(),
					inf.getPubOrSub(), inf.getEventHandler());
		}
		...
		for (EventPubSubInfo inf : subs)
		{
			if (!fully && filter_all_but_eval_eps(inf))
				continue;
			addEventPubSub(inf.getEventType(), inf.getPubOrSub(),
					link.getCell(), inf.getEventHandler());
		}
	}

	private static boolean filter_all_but_eval_eps(EventPubSubInfo inf)
	{
		if (!EvalCellEvent.HANDLE.equals(inf.getEventType()))
			return true;
		if (!(ThisNiche.graph
				.get(inf.getPubOrSub()) instanceof CellGroupMember))
			return true;
		if (!(ThisNiche.graph
				.get(inf.getEventHandler()) instanceof CellGroupMember))
			return true;
		return false;
	}
	...
	{
		List<NotebookDocument> docs = hg.getAll(ThisNiche.graph,
		...
		List<BackupLink> res = hg.getAll(ThisNiche.graph,
				hg.type(BackupLink.class));
	...
	{
		HGHandle any = ThisNiche.graph.getHandleFactory().anyHandle();
		List<EventPubSub> pubs = hg.getAll(ThisNiche.graph, hg.and(
				hg.type(EventPubSub.class), hg.incident(cell),
				hg.orderedLink(new HGHandle[] { any, cell, any, any })));
		List<EventPubSubInfo> pubs_out = new ArrayList<EventPubSubInfo>(
				pubs.size());
		for (EventPubSub eps : pubs)
			pubs_out.add(new EventPubSubInfo(eps.getEventType(),
					eps.getSubscriber(), eps.getEventHandler()));
		List<EventPubSub> subs = hg.getAll(ThisNiche.graph, hg.and(
				hg.type(EventPubSub.class), hg.incident(cell),
				hg.orderedLink(new HGHandle[] { any, any, cell, any })));
		List<EventPubSubInfo> subs_out = new ArrayList<EventPubSubInfo>(
				pubs.size());
		for (EventPubSub eps : subs)
			subs_out.add(new EventPubSubInfo(eps.getEventType(),
					eps.getPublisher(), eps.getEventHandler()));
	...
	public static HGHandle addSerializable(Object o)
	{
					...
					&& !(o instanceof NotebookUI))
			{
				...
				catch (Throwable ex)
				{
					...
					catch (Throwable tt)
					{
						...
						// tt.toString());
						h = ThisNiche.graph
								.add(new NotSerializableValue().value(o));
					}
				}
				finally
				{
		...
		{
			System.err.println(
					"Unable to add Cell value: " + o + " Reason: " + ex);
			ex.printStackTrace();
			h = ThisNiche.graph.add(ex.toString());
		}
		finally
	...
	public static void fireCellTextChanged(HGHandle cellH,
			CellTextChangeEvent e)
	{
