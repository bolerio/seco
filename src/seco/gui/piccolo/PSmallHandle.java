...
/**
 * <b>PSmallHandle</b> is used to modify some aspect of Piccolo when it is
 * dragged. Each handle has a PLocator that it uses to automatically position
...
public class PSmallHandle extends PNodeEx
{
    ...
    // public static Paint DEFAULT_PAINT = new GradientPaint( 0, 0, Color.white,
    // 2, 2, Color.black, true );
    ...
    protected void installHandleEventHandlers()
    {
        handleDragger = new PDragSequenceEventHandler()
        {
            protected void startDrag(PInputEvent event)
            {
                ...
                startHandleDrag(event.getPositionRelativeTo(PSmallHandle.this),
                                event);
            }
            ...
            {
                super.endDrag(event);
                endHandleDrag(event.getPositionRelativeTo(PSmallHandle.this),
                              event);
            }
        };
        ...
        handleDragger.setEventFilter(new PInputEventFilter(
        ...
        handleDragger.getEventFilter().setAcceptsMouseMoved(false); // no need
                                                                    // for moved
                                                                    // events
                                                                    // for
                                                                    // handle
