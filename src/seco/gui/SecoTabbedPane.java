...
public class SecoTabbedPane extends JTabbedPane
{
    ...
    private static class DragMouseListener implements
            MouseMotionListener
    {
    ...
    public static class TPTransferHandler extends TransferHandler
    {
        ...
        protected DataFlavor getImportFlavor(DataFlavor[] flavors, JComponent c)
        {
            for (int i = 0; i < flavors.length; i++)
                if (flavors[i].equals(SecoTransferable.nbFlavor)) return flavors[i];
            return null;
        }
       ...
       public int getSourceActions(JComponent c)
        ...
        public boolean importData(JComponent comp, Transferable t)
        {
            ...
            DataFlavor importFlavor = getImportFlavor(t.getTransferDataFlavors(), comp);
            ...
            try
            {
                if (importFlavor.equals(SecoTransferable.nbFlavor))
                {
                    ...
                    if(in instanceof NotebookUI)
                    {
                        ...
                        GUIHelper.addTabToTabbedPaneGroup(
                                ui.getDoc().getHandle());
                        return true;
                    }
                    String title = (in.getName() != null && in.getName().length() > 0) ?
                            in.getName() : "Some Component";
                    tp.addTab(title, in);
                }
            }
            catch (Exception ioe)
