...
public class AstPath implements Iterable<ASTNode>
{
    ...
    public AstPath(ASTNode root, int caretOffset, Element el)
    {
    ...
    public int[] offsetToLineCol(Element el, int offset)
    {
        int[] res = new int[2];
    ...
    public AstPath(ASTNode node, ASTNode target)
    {
        ...
        else
        {
            ...
            // When I get time rewrite the find method to build the list that
    ...
    /**
     * Find the position closest to the given offset in the AST. Place the path
     * from the leaf up to the path in the passed in path list.
     */
    @SuppressWarnings("unchecked")
    private ASTNode findPathTo(ASTNode node, int line, int column)
    {
        ...
        if (!path.isEmpty() && (path.get(0) instanceof ClassNode))
        {
            ...
            if (clazz.isScript()
                    && (path.size() == 1 || path.get(1) instanceof Expression || path
                            .get(1) instanceof Statement))
            {
                ...
                if (method != null)
                {
                    if (method.getCode() != null
                            && (path.size() <= 1 || method.getCode() != path
                                    .get(1)))
                    {
    ...
    /**
     * Return an iterator that returns the elements from the leaf back up to the
     * root
     */
