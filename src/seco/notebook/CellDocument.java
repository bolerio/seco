...
public class CellDocument extends NotebookDocument
{

    public CellDocument(HGHandle h)
    {
        super(h);
    }
    ...
    public HGHandle getHandle()
    {
        ...
        Set<HGHandle> list = CellUtils.findAll(ThisNiche.hg, hg
                .type(CellDocument.class));
        for (HGHandle h : list)
            if (bookH.equals(((CellDocument) ThisNiche.hg.get(h)).bookH)) return handle = h;
