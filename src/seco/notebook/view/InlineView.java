...
public class InlineView extends LabelView // implements HidableView
{
	...
	{
		if (customPainter == null) customPainter = new GlyphPainter0();
	...
	public void paint(Graphics g, Shape a)
	{
		...
		if (!paintedText) paintTextUsingColor0(g, a, fg, p0, p1);
	}
	...
	final void paintTextUsingColor0(Graphics g, Shape a, Color c, int p0, int p1)
	{
		...
		if (underline || strike)
		{
			...
			if (p != p0)
			{
				x0 += (int) customPainter.getSpan(this, p, p0, getTabExpander(), x0);
			}
			int x1 = x0
					+ (int) customPainter.getSpan(this, p0, p1, getTabExpander(), x0);
			// calculate y coordinate
			//int d = (int) customPainter.getDescent(this);
			int y = alloc.y + alloc.height - (int) customPainter.getDescent(this);
			if (underline)
			{
				int yTmp = y;
				yTmp += 1;
