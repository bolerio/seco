...
public class PSwingEventHandler implements PInputEventListener
{
     ...
     * 
     * @param canvas
     *            the canvas associated with this PSwingEventHandler.
     * @param node
     *            the node the mouse listeners will be attached to.
     */
        ...
        if (c instanceof Container)
        {
            ...
            for (int i = 0; i < ncomponents; i++)
            {
                ...
                if (comp != null && comp.isVisible() && comp.isEnabled())
                {
                    ...
                    {
                        comp = findShowingComponentAt(comp, x - (int) p.getX(),
                                y - (int) p.getY());
                    } else
                    {
                    ...
                    if (comp != null && comp.isShowing()) { return comp; }
                }
            }
        }
        return c;
    ...
    void dispatchEvent(PSwingMouseEvent pSwingMouseEvent, PInputEvent aEvent)
    {
        ...
        if (currentNode instanceof PSwing)
        {
            ...
            if (grabNode.isDescendentOf(canvas.getRoot()))
            {
                ...
                comp = findShowingComponentAt(swing.getComponent(), (int) pt
                        .getX(), (int) pt.getY());
                ...
                if (comp != null && comp != swing.getComponent())
                {
                    for (Component c = comp; c != null
                            && c != swing.getComponent(); c = c.getParent())
                    {
                        ...
                        && pSwingMouseEvent.getID() == MouseEvent.MOUSE_PRESSED)
                {
                    if (SwingUtilities.isLeftMouseButton(pSwingMouseEvent))
                    {
                        leftButtonData.setState(swing, pickedNode, comp, offX,
                            ...
                            .isMiddleMouseButton(pSwingMouseEvent))
                    {
                        middleButtonData.setState(swing, pickedNode, comp,
                                offX, offY);
                    } else if (SwingUtilities
                            .isRightMouseButton(pSwingMouseEvent))
                    {
                ...
                && (comp != null))
        {
            ...
            MouseEvent e_temp = new MouseEvent(comp, pSwingMouseEvent.getID(),
                    ...
                    pSwingMouseEvent.getModifiers(), (int) pt.getX() - offX,
                    (int) pt.getY() - offY, pSwingMouseEvent.getClickCount(),
                    pSwingMouseEvent.isPopupTrigger());

            PSwingMouseEvent e2 = PSwingMouseEvent.createMouseEvent(e_temp
                    .getID(), e_temp, aEvent);
            dispatchEvent(comp, e2);
            pSwingMouseEvent.consume();
        } else if (pSwingMouseEvent.getID() == MouseEvent.MOUSE_WHEEL)
        {
           // System.out.println("PSwingEventHandler -dispatchEvent: " + aEvent
           //         + ":" + currentNode);
            MouseWheelEvent evt = (MouseWheelEvent) aEvent.getSourceSwingEvent();
//            MouseEvent e_temp = new MouseWheelEvent(comp, pSwingMouseEvent.getID(),
//                    pSwingMouseEvent.getWhen(),
//                    pSwingMouseEvent.getModifiers(), (int) pt.getX() - offX,
//                    (int) pt.getY() - offY, pSwingMouseEvent.getClickCount(),
//                    pSwingMouseEvent.isPopupTrigger(), evt.getScrollType(), evt.getScrollAmount(), evt.getWheelRotation());
            dispatchEvent(comp, evt);
            pSwingMouseEvent.consume();
        }
        ...
        } else
        {
            ...
            if (comp != null)
            {
                MouseEvent e_temp = createEnterEvent(comp, pSwingMouseEvent,
                        offX, offY);
                PSwingMouseEvent e2 = PSwingMouseEvent.createMouseEvent(e_temp
                        .getID(), e_temp, aEvent);
                dispatchEvent(comp, e2);
    ...
    private MouseEvent createEnterEvent(Component comp, PSwingMouseEvent e1,
            int offX, int offY)
    {
            ...
            ButtonData buttonData)
    {
        ...
        if (buttonData.getPNode().isDescendentOf(canvas.getRoot()))
        {
            ...
            MouseEvent e_temp = new MouseEvent(
                    buttonData.getFocusedComponent(), e1.getID(), e1.getWhen(),
                    e1.getModifiers(), (int) pt.getX()
                            - buttonData.getOffsetX(), (int) pt.getY()
                            - buttonData.getOffsetY(), e1.getClickCount(), e1
                            .isPopupTrigger());
    ...
    private void dispatchEvent(final Component target, final MouseEvent event)
    {
        // if(event.getID() != MouseEvent.MOUSE_MOVED)
        // System.out.println("dispatchEvent: " + event + ";" + target);
        if(target != null)
    ...
    public void processEvent(PInputEvent aEvent, int type)
    {
        ...
        if (aEvent.isMouseEvent())
        {
            ...
            if (sourceSwingEvent instanceof MouseEvent)
            {
                ...
                PSwingMouseEvent pSwingMouseEvent = PSwingMouseEvent
            ...
            System.out.println("PSwingEventHandler: " + aEvent);

        /*
         * if( !( EventQueue.getCurrentEvent() instanceof MouseEvent ) ) { new
         * Exception( "EventQueue.getCurrentEvent was not a MouseEvent, consider
    ...
    private static class ButtonData
    {
        ...
        public void setState(PSwing swing, PNode visualNode, Component comp,
                int offX, int offY)
        {
            focusPSwing = swing;
            focusComponent = comp;
