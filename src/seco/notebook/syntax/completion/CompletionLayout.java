...
public final class CompletionLayout
{
    ...
    /**
     * Visual shift of the completion window to the left so that the text in the
     * rendered completion items.aligns horizontally with the text in the
     * document.
     */
    ...
    {
        return (editorComponentRef != null) ? (JTextComponent) editorComponentRef
                .get()
                : null;
    }
    ...
    public void processKeyEvent(KeyEvent evt)
    {
        ...
        {
            CompletionLayoutPopup popup = (CompletionLayoutPopup) visiblePopups
                    .get(i);
    ...
    public void showDocumentation(CompletionDocumentation doc, int anchorOffset)
    {
        ...
        if (!visiblePopups.contains(docPopup)) visiblePopups.push(docPopup);
    }
    ...
    public void showToolTip(JToolTip toolTip, int anchorOffset)
    {
        ...
        if (!visiblePopups.contains(tipPopup)) visiblePopups.push(tipPopup);
    }
    ...
    /**
     * Layout either of the copmletion, documentation or tooltip popup. <br>
     * This method can be called recursively to update other popups once certain
     * popup was updated.
     * 
     * <p>
    ...
    void updateLayout(CompletionLayoutPopup popup)
    {
        ...
        { // completion popup
            if (isToolTipVisible())
            {
                ...
                { // not enough space -> show on same side
            ...
            if (docPopup.isVisible()
                    && (docPopup.isOverlapped(popup)
                            || docPopup.isOverlapped(tipPopup)
                            || docPopup.getAnchorOffset() != completionPopup
                                    .getAnchorOffset() || !docPopup
                            .isShowRetainedPreferredSize()))
            {
                updateLayout(docPopup);
            }

        }
        else if (popup == docPopup)
        { // documentation popup
            ...
            Rectangle occupiedBounds = popup.getAnchorOffsetBounds();
            occupiedBounds = tipPopup.unionBounds(completionPopup
                   .unionBounds(occupiedBounds));
            
            docPopup.showAlongOccupiedBounds(occupiedBounds);
            if (completionPopup.isOverlapped(popup))
            {    
                if(docPopup.isDisplayAboveCaret())
                {
                    occupiedBounds = docPopup.getPopupBounds();
                    occupiedBounds.y = 
                        completionPopup.getPopupBounds().y -
                        docPopup.getPopupBounds().height;
                    docPopup.show(occupiedBounds, true);
                }
            }

        }
        else if (popup == tipPopup)
        { // tooltip popup
            popup.showAlongAnchorBounds(); // show possibly above the caret
            if (completionPopup.isOverlapped(popup)
                    || docPopup.isOverlapped(popup))
            {
                // docPopup layout will be handled as part of completion popup
                // layout
    ...
    private static final class CompletionPopup extends CompletionLayoutPopup
    {
                ...
                ListSelectionListener listSelectionListener)
        {
            ...
            { // not yet visible => create completion scrollpane
                lastSize = new Dimension(0, 0); // no last size => use (0,0)

                setContentComponent(new CompletionScrollPane(editorComponent,
                        listSelectionListener, new MouseAdapter() {
                            public void mouseClicked(MouseEvent evt)
                            {
                                ...
                                if (SwingUtilities.isLeftMouseButton(evt))
                                {
                                    ...
                                    {
                                        CompletionItem selectedItem = getCompletionScrollPane()
        ...
        public CompletionItem getSelectedCompletionItem()
        {
            return isVisible() ? getCompletionScrollPane()
                    .getSelectedCompletionItem() : null;
        }
        ...
        public void processKeyEvent(KeyEvent evt)
        {
            if (isVisible())
            {
                Object actionMapKey = getCompletionScrollPane().getInputMap()
                        .get(KeyStroke.getKeyStrokeForEvent(evt));

                if (actionMapKey != null)
                {
                    Action action = getCompletionScrollPane().getActionMap()
                            .get(actionMapKey);
                    if (action != null)
                    {
                        action.actionPerformed(new ActionEvent(
                                getCompletionScrollPane(), 0, null));
                        evt.consume();
                    }
                }
            }
    ...
    private static final class DocPopup extends CompletionLayoutPopup
    {
        ...
        protected void show(CompletionDocumentation doc, int anchorOffset)
        {
            ...
            if (!isVisible())
            { // do not check for size as it should remain the same
        ...
        public void processKeyEvent(KeyEvent evt)
        {
            if (isVisible())
            {
                Object actionMapKey = getDocumentationScrollPane()
                        .getInputMap().get(KeyStroke.getKeyStrokeForEvent(evt));

                if (actionMapKey != null)
                {
                    Action action = getDocumentationScrollPane().getActionMap()
                            .get(actionMapKey);
                    if (action != null)
                    {
                        action.actionPerformed(new ActionEvent(
                                getDocumentationScrollPane(), 0, null));
                        evt.consume();
                    }
                }
            }
    ...
    private static final class TipPopup extends CompletionLayoutPopup
    {
        ...
        protected void show(JToolTip toolTip, int anchorOffset)
        {
            ...
            if (!getPreferredSize().equals(lastSize))
            { // preferred sizes differ
                getLayout().updateLayout(this);
            }
        }
