...
import seco.notebook.syntax.completion.CompletionQuery.ResultItem;
		...
		CompletionItem
{
	...
	public static class MethodResultItem extends CallableFeatureResultItem
	{
		...
		public MethodResultItem(String mtdName, Class type, Class[] params,
				Class[] exc)
		{
			super(mtdName);
			...
			JavaResultItem
	{
		...
		protected void populateParamsAndExceptions(Class[] prms, Class[] exceps)
		{
			if (prms != null)
				for (int i = 0; i < prms.length; i++)
				{
					params.add(new ParamStr(prms[i].getSimpleName(), JMIUtils
							.getTypeName(prms[i], false, false), "arg" + i,
