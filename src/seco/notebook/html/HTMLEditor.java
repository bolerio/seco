...
public class HTMLEditor extends JTextPane 
{
	...
	public HTMLEditor(String filename)
	{
		setEditable(true);
		...
		try
		{
			url = new File(filename).getParentFile().toURL();
		}
		catch (Exception ex)
		{
		}
		if (url != null) getDoc().setBase(url);
	...
	protected void installKeyActions()
	{
		...
		//inputMap.put(key, htmlKit.getActionByName(MyHTMLEditorKit.fontAction));
			...
			boolean setParagraphAttributes)
	{
		...
		if (setParagraphAttributes)
			getDoc().setParagraphAttributes(xStart, xFinish - xStart, attr,
					false);
		else
		{
			if (xStart != xFinish)
				getDoc().setCharacterAttributes(xStart, xFinish - xStart, attr,
						false);
			else
			{
	...
	void removeListItem(Element el) throws BadLocationException,
			IOException
	{
	...
	void insertListItem(Element element, String s)
	...
	void insertListItem(HTML.Tag baseTag,String text) throws BadLocationException, IOException
	{
		Element el = HTMLUtils.getTag(editor, HTML.Tag.LI,
			...
			Dimension size)
	{
		...
		if (linkImage == null)
			setTextLink(e, href, linkText, doc);
		else
	 ...
	 * @param e the link element found at the selection, or null if none was
	 * found
