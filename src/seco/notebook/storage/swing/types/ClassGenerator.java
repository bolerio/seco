...
import seco.notebook.storage.swing.GenUtils;

public class ClassGenerator
{
    private static final boolean WRITE_TO_DISK = !false;
    private static final String GENERATED_CLASSES_OUTPUT_DIR;
    static
    {
        if (WRITE_TO_DISK)
        {
    ...
    protected void generateCtr(ClassWriter cw)
    {
        ...
        mv.visitMethodInsn(INVOKESPECIAL,
                "seco/notebook/storage/swing/types/GeneratedClass", "<init>",
                "()V");
        mv.visitInsn(RETURN);
        Label l1 = new Label();
        mv.visitLabel(l1);
    ...
    public Class<?> generate()
    {
        ...
        generateCtr(cw);
        genInstantiate(cw);
        if (!inspector.isEmptyMakeMethod()) genMake(cw);
        if (!inspector.isEmptyStoreMethod()) genStore(cw);
        cw.visitEnd();
        ...
        if (WRITE_TO_DISK)
            try
            {
                FileOutputStream fos = new FileOutputStream(
                        GENERATED_CLASSES_OUTPUT_DIR + genClassName + ".class");
                fos.write(byteCode);
                fos.close();
            }
            catch (Exception ex)
    ...
    protected void genMake(ClassWriter cw)
    {
        ...
        for (Field f : inspector.getPrivFieldsMap().values())
        {
            ...
            if (inspector.addons != null
                    && !inspector.addons.containsKey(f.getName()))
                genMakePrivField(mv, f);
        }
        for (EventSetDescriptor e : inspector.getEventSetDescriptorsMap()
                .values())
    ...
    protected void genStore(ClassWriter cw)
    {
        ...
        for (String s : inspector.getEventSetDescriptorsMap().keySet())
        {
            ...
            if (e != null && !filterListenersByType(e.getListenerType()))
                genStoreListeners(mv, s, inspector.getEventSetDescriptorsMap()
                        .get(s));
        }
        for (Field f : inspector.getPrivFieldsMap().values())
    ...
    {
        if (listenerType == java.awt.event.ComponentListener.class) { return true; }
        ...
        if (listenerType == javax.swing.event.ChangeListener.class
                && type.getJavaClass() == javax.swing.JMenuItem.class) { return true; }
        return false;
    }
            ...
            EventSetDescriptor e)
    {
        ...
        mv
                .visitMethodInsn(INVOKEVIRTUAL, BASE_CLS_NAME,
                        "filterListeners",
    ...
    protected void genStorePubField(MethodVisitor mv, Field f)
    {
        ...
        {
            mv.visitFieldInsn(GETFIELD, className, f.getName(), Type.getType(t)
                    .getDescriptor());
        }
        mv.visitMethodInsn(INVOKEVIRTUAL, BASE_CLS_NAME, "setValue",
                SET_VALUE_DESC);
    ...
    protected void genMakePubField(MethodVisitor mv, Field f)
    {
        ...
        else
        {
            ...
            mv.visitFieldInsn(PUTFIELD, className, f.getName(), Type.getType(t)
                    .getDescriptor());
        }
        mv.visitLabel(l0);
    }
            ...
            String name)
    {
        ...
        mv.visitVarInsn(ALOAD, 2);
        mv.visitVarInsn(ALOAD, 4);
        mv.visitMethodInsn(INVOKEVIRTUAL, BASE_CLS_NAME, "getValue",
                GET_VALUE_DESC);
    ...
    protected void genInstantiate(ClassWriter cw)
    {
        ...
        Constructor<?> ctr = (ctr_or_m instanceof Constructor<?>) ? (Constructor<?>) ctr_or_m
                : null;
        if (ctr != null && !Modifier.isPublic(ctr.getModifiers())) return;
        ...
        else
        {
            ...
            Class<?>[] types = (ctr != null) ? ctr.getParameterTypes() : m
                    .getParameterTypes();
            if (ctr != null)
            {
                mv.visitTypeInsn(NEW, className);
            ...
            if (ctr != null) mv.visitMethodInsn(INVOKESPECIAL, className,
                    "<init>", Type.getConstructorDescriptor(ctr));
            else
            {
                mv.visitMethodInsn(INVOKESTATIC, Type.getType(
                        m.getDeclaringClass()).getInternalName(), m.getName(),
    ...
    protected void genInstVar(MethodVisitor mv, String name, Class<?> c)
    {
        ...
        mv.visitLdcInsn(name);
        mv
                .visitMethodInsn(INVOKEVIRTUAL, BASE_CLS_NAME, "getValue",
    ...
    private void genDefaultCellEditorCtr(ClassWriter cw)
    {
        ...
        mv
                .visitMethodInsn(INVOKEVIRTUAL, className, "getValue",
                        "(Lorg/hypergraphdb/type/Record;Ljava/lang/String;)Ljava/lang/Object;");
