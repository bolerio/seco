...
public class VisPropsProvider implements DynamicMenuProvider, Serializable
{
	...
	private static final class FormatAction implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			DialogDescriptor dd = new DialogDescriptor(AppForm.getInstance(), outer,
	...
	private static class SyntaxStyleAction implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			NotebookUI ui = app.getCurrentNotebook();
			if (ui == null) return;
			ScriptSupport sup = ui.getDoc().getScriptSupport(
					ui.getCaretPosition());
			if (sup == null) return;
			SyntaxHiliteOptionPane pane = new SyntaxHiliteOptionPane(sup);
			SettingsPreviewPane outer = new SettingsPreviewPane(ui
					.getDoc(), pane, null);
			DialogDescriptor dd = new DialogDescriptor(app, outer,
					"Syntax Styles");
			if (DialogDisplayer.getDefault().notify(dd) == NotifyDescriptor.OK_OPTION)
			{
				outer.save();
	...
	static class MIActionListener extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent evt)
		{
			...
			NotebookUI ui = app.getCurrentNotebook();
			if(ui == null) return;
			CellPropsDialog dlg = new CellPropsDialog(app,
					ui.getDoc(), stype);
			dlg.setVisible(true);
			if (dlg.succeeded()) {
			    ui.revalidate();
